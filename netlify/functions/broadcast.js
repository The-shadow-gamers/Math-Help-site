...
exports.handler = async (event) => {
  ...
  // Persist event in Netlify Blobs (ring buffer-like)
  const store = getStore("mh-events");
  const seqKey = "seq";
  const now = Date.now();

  let seqRaw = await store.get(seqKey);
  let seq = Number(seqRaw || 0) + 1;

  const eventObj = { id: seq, ts: now, payload };
  await store.set(`e/${seq}`, JSON.stringify(eventObj));
  await store.set(seqKey, String(seq));

  // Optional: keep current "state" for late joiners (only for message overlays/banners)
  if (payload.type === "message") {
